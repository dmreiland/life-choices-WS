...
public class MeetupService {
    ...
    public String getEvents(Double latitude, Double longitude, String category, Integer page, Integer radius) throws IOException {
        if(radius != null &&    radius > MEETUP_RADIUS) {
            radius = MEETUP_RADIUS;
        }
        
        String url =  String.format("%s/open_events?format=json&lat=%s&lon=%s&page=%s&desc=true%s%s&key=%s",MEETUP_API_URL,latitude,longitude,page,
                category != null ? "&category=" + category : "", radius != null ? "&radius=" + radius : "", MEETUP_API_KEY);
